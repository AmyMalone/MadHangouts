...
* the License.
*/
var serverPath = '//stone-bison-828.appspot.com/';
...
function resetButtonClick() {
  ...
  gapi.hangout.data.submitDelta({'w2': 'No words entered yet'});
}
...
function wordButtonClick() {
  ...
  if(document.getElementById("searchTxt").value){
    var w2 = document.getElementById("searchTxt").value;
  }else{
    var w2 = 'oops';
  }
  ...
  console.log('New word is ' + value);
  var value = value.bold();
  // Send update to shared state.
  // NOTE:  Only ever send strings as values in the key-value pairs
  gapi.hangout.data.submitDelta({'w2': 'IT is a truth universally acknowledged, that a '+value+' man in possession of a large fortune must be in want of a wife. --Jane Austin'});
}
...
function updateStateUi(state) {
  ...
  var wordElement = document.getElementById('w2');
  var stateWord = state['w2'];
  if (!stateWord) {
    setText(wordElement, 'No words entered yet');
  } else {
    setText(wordElement, stateWord.toString());
  }
}
